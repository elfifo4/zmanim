...
import com.kosherjava.zmanim.hebrewcalendar.YerushalmiYomiCalculator.getDafYomiYerushalmi
import com.kosherjava.zmanim.hebrewcalendar.YomiCalculator.getDafYomiBavli
...
class JewishCalendar : JewishDate {
    ...
    /**
     * A constructor that initializes the date to the [Date][java.util.Date] parameter.
     *
     * @param date
    ...
    /**
     * A constructor that initializes the date to the [Calendar][java.util.Calendar] parameter.
     *
     * @param calendar
    ...
    /**
     * A constructor that initializes the date to the [LocalDate][java.time.LocalDate] parameter.
     *
     * @param localDate
     ...
     */
    constructor(jewishYear: Int, jewishMonth: Int, jewishDayOfMonth: Int) : super(jewishYear, jewishMonth, jewishDayOfMonth) {}
    ...
    val isBirkasHachamah: Boolean
        get() {
            var elapsedDays = getJewishCalendarElapsedDays(getJewishYear()) //elapsed days since molad ToHu
            elapsedDays = elapsedDays + daysSinceStartOfJewishYear //elapsed days to the current calendar date
            ...
            /* Molad Nissan year 1 was 177 days after molad tohu of Tishrei. We multiply 29.5 days * 6 months from Tishrei
              ...
              * Rosh Hashana as 1, we have to add 1 day for a total of 171. To this add a day since the tekufah is on a Tuesday
              * night and we push off the bracha to Wednesday AM resulting in the 172 used in the calculation.
              */return if (elapsedDays % (28 * 365.25) == 172.0) { // 28 years of 365.25 days + the offset from molad tohu mentioned above
                true
            } else false
        }//ZaSh
...
//ZaCh//Hak//HaSh//BaSh//BaCh//not a leap year//ZaSh//ZaCh//HaSh//HaCh//BaSh//BaCh// convert 0 to 7 for Shabbos for readability// plus one to the original Rosh Hashana of year 1 to get a week starting on Sunday
    ...
    val parshah: Parsha
        get() {
            ...
            val roshHashanaDayOfWeek = getJewishCalendarElapsedDays(getJewishYear()) % 7
            val day = roshHashanaDayOfWeek + daysSinceStartOfJewishYear
            return if (yearType >= 0) { // negative year should be impossible, but let's cover all bases
                parshalist[yearType][day / 7]
            } else Parsha.NONE
            //keep the compiler happy
    ...
    val upcomingParshah: Parsha
        get() {
            val clone = clone() as JewishCalendar
...
// push off Tzom Gedalia if it falls on Shabbos// if Tisha B'av falls on Shabbos, push off until Sunday// push off the fast day if it falls on Shabbos// if 5 Iyar falls on Wed, Yom Haatzmaut is that day. If it fal1s on Friday or Shabbos, it is moved back to
    ...
    val yomTovIndex: Int
        get() {
            ...
            when (getJewishMonth()) {
                ...
                IYAR -> {
                    if (isUseModernHolidays && ((day == 4 && dayOfWeek == Calendar.TUESDAY || day == 3 || day == 2) && dayOfWeek == Calendar.WEDNESDAY || day == 5) && dayOfWeek == Calendar.MONDAY) {
                    ...
                    if (isUseModernHolidays
                        && ((day == 5 && dayOfWeek == Calendar.WEDNESDAY)
                                || ((day == 4 || day == 3) && dayOfWeek == Calendar.THURSDAY)
                ...
                ADAR -> if (!isJewishLeapYear) {
                    // if 13th Adar falls on Friday or Shabbos, push back to Thursday
                    if ((day == 11 || day == 12) && dayOfWeek == Calendar.THURSDAY || day == 13 && !(dayOfWeek == Calendar.FRIDAY || dayOfWeek == Calendar.SATURDAY)) {
    ...
    val isYomTov: Boolean
        get() {
            ...
            return if ((isErevYomTov && (holidayIndex != HOSHANA_RABBA || holidayIndex == CHOL_HAMOED_PESACH) && jewishDayOfMonth != 20 || isTaanis) && holidayIndex != YOM_KIPPUR || holidayIndex == ISRU_CHAG) {
                false
            } else yomTovIndex != -1
        }
    ...
    val isYomTovAssurBemelacha: Boolean
        get() {
            val holidayIndex = yomTovIndex
            return holidayIndex == PESACH || holidayIndex == SHAVUOS || holidayIndex == SUCCOS || holidayIndex == SHEMINI_ATZERES || holidayIndex == SIMCHAS_TORAH || holidayIndex == ROSH_HASHANA || holidayIndex == YOM_KIPPUR
        }
    ...
    val isErevYomTovSheni: Boolean
        get() = (getJewishMonth() == TISHREI && (jewishDayOfMonth == 1))
                || (!inIsrael
    ...
    val isAseresYemeiTeshuva: Boolean
        get() = getJewishMonth() == TISHREI && jewishDayOfMonth <= 10
    ...
    val isErevYomTov: Boolean
        get() {
            val holidayIndex = yomTovIndex
            return holidayIndex == EREV_PESACH || holidayIndex == EREV_SHAVUOS || holidayIndex == EREV_ROSH_HASHANA || holidayIndex == EREV_YOM_KIPPUR || holidayIndex == EREV_SUCCOS || holidayIndex == HOSHANA_RABBA || holidayIndex == CHOL_HAMOED_PESACH && jewishDayOfMonth == 20
    ...
    val isErevRoshChodesh: Boolean
        get() =// Erev Rosh Hashana is not Erev Rosh Chodesh.
            jewishDayOfMonth == 29 && getJewishMonth() != ELUL
    ...
    val isYomKippurKatan: Boolean
        get() {
            ...
            return if ((day == 27 || day == 28) && dayOfWeek == Calendar.THURSDAY) {
                true
            } else false
        }
 ...
 * unknown Days: BeHaB Vs. Pesach Sheini?](https://ohr.edu/this_week/insights_into_halacha/9340) there are some customs that delay the day to various points in the future.
    ...
    val isBeHaB: Boolean
        get() {
            ...
            if (month == CHESHVAN || month == IYAR) {
                if ((dayOfWeek == Calendar.MONDAY && day > 4 && day < 18) || (dayOfWeek == Calendar.THURSDAY && day > 7 && day < 14)) {
                    return true
                }
            }
            return false
    ...
    val isTaanisBechoros: Boolean
        get() {
            ...
            val dayOfWeek = dayOfWeek
            // on 14 Nissan unless that is Shabbos where the fast is moved back to Thursday
            return getJewishMonth() == NISSAN && (day == 14 && dayOfWeek != Calendar.SATURDAY || day == 12) && dayOfWeek == Calendar.THURSDAY
        }// teves
        ...
        get() {
            ...
            return if (isChanukah) {
                if (getJewishMonth() == KISLEV) {
                    ...
                    if (isKislevShort) day + 5 else day + 6
                }
            } else {
                -1
            }
    ...
    val isShabbosMevorchim: Boolean
        get() = dayOfWeek == Calendar.SATURDAY && jewishDayOfMonth >= 23 && jewishDayOfMonth <= 29 && getJewishMonth() != ELUL// if Sivan and before Shavuos// if Iyar// not a day of the Omer
...
// Har Habayis latitude
    ...
    val moladAsDate: Date
        get() {
            ...
            // being incorrectly off by an hour in the summer due to DST. Proper adjustment for the actual time in DST will be done by the date
            // formatter class used to display the Date.
            val yerushalayimStandardTZ = TimeZone.getTimeZone("GMT+2")
            val geo = GeoLocation(locationName, latitude, longitude, yerushalayimStandardTZ)
            val cal = Calendar.getInstance(geo.getTimeZone())
            cal.clear()
            val moladSeconds = molad.moladChalakim * 10 / 3.0
            cal[molad.getGregorianYear(), molad.getGregorianMonth(), molad.getGregorianDayOfMonth(), molad.moladHours, molad.moladMinutes] =
                moladSeconds.toInt()
    ...
    val dafYomiYerushalmi: Daf?
        get() = getDafYomiYerushalmi(this)// Days since Rosh Hashana year 1. Add 1/2 day as the first tekufas tishrei was 9 hours into the day. This allows all
    ...
    val tekufasTishreiElapsedDays: Int
        get() {
            ...
            // 4 years of the secular leap year cycle to share 47 days. Truncate 47D and 9H to 47D for simplicity.
            val days = getJewishCalendarElapsedDays(getJewishYear()) + (daysSinceStartOfJewishYear - 1) + 0.5
            // days of completed solar years
            val solar = (getJewishYear() - 1) * 365.25
...
//Not recited on Friday night// The 7th Cheshvan can't occur on Shabbos, so always return true for 7 Cheshvan
    ...
    @get:Deprecated(
        """Use {@link TefilaRules#isVeseinTalUmatarStartDate(JewishCalendar)} instead. This method will be
    ...
    val isVeseinTalUmatarStartDate: Boolean
        get() {
            if (inIsrael) {
                ...
                if (dayOfWeek == Calendar.SATURDAY) { //Not recited on Friday night
                    return false
                }
                return if (dayOfWeek == Calendar.SUNDAY) { // When starting on Sunday, it can be the start date or delayed from Shabbos
                    tekufasTishreiElapsedDays == 48 || tekufasTishreiElapsedDays == 47
                } else {
                    tekufasTishreiElapsedDays == 47
                }
            }
     ...
     */  // (forRemoval=true) // add back once Java 9 is the minimum supported version
    @get:Deprecated(
        """Use {@link TefilaRules#isVeseinTalUmatarStartingTonight(JewishCalendar)} instead. This method
    ...
    val isVeseinTalUmatarStartingTonight: Boolean
        get() {
            if (inIsrael) {
                ...
                if (dayOfWeek == Calendar.FRIDAY) { //Not recited on Friday night
                    return false
                }
                return if (dayOfWeek == Calendar.SATURDAY) { // When starting on motzai Shabbos, it can be the start date or delayed from Friday night
                    tekufasTishreiElapsedDays == 47 || tekufasTishreiElapsedDays == 46
                } else {
                    tekufasTishreiElapsedDays == 46
                }
            }
            return false
        }
     ...
     */  // (forRemoval=true) // add back once Java 9 is the minimum supported version
    @get:Deprecated(
        """Use {@link TefilaRules#isVeseinTalUmatarRecited(JewishCalendar)} instead. This method will
    ...
    val isVeseinTalUmatarRecited: Boolean
        get() {
            ...
            return if (inIsrael) {
                getJewishMonth() != CHESHVAN || jewishDayOfMonth >= 7
            } else {
                tekufasTishreiElapsedDays >= 47
            }
        }
     ...
     */  // (forRemoval=true) // add back once Java 9 is the minimum supported version
    @get:Deprecated(
        """Use {@link TefilaRules#isVeseinBerachaRecited(JewishCalendar)} instead. This method will be
	          removed in the v3.0 release.
     ...
     */  // (forRemoval=true) // add back once Java 9 is the minimum supported version
    @get:Deprecated(
        """Use {@link TefilaRules#isMashivHaruachStartDate(JewishCalendar)} instead. This method will be
	          removed in the v3.0 release.
	  
	  """
    )
    val isMashivHaruachStartDate: Boolean
        get() = getJewishMonth() == TISHREI && jewishDayOfMonth == 22
     ...
     */  // (forRemoval=true) // add back once Java 9 is the minimum supported version
    @get:Deprecated(
        """Use {@link TefilaRules#isMashivHaruachEndDate(JewishCalendar)} instead. This method will be
	          removed in the v3.0 release.
	  
	  """
    )
    val isMashivHaruachEndDate: Boolean
        get() = getJewishMonth() == NISSAN && jewishDayOfMonth == 15
     ...
     */  // (forRemoval=true) // add back once Java 9 is the minimum supported version
    @get:Deprecated(
        """Use {@link TefilaRules#isMashivHaruachRecited(JewishCalendar)} instead. This method will be
	          removed in the v3.0 release.
    ...
    val isMashivHaruachRecited: Boolean
        get() {
            val startDate = JewishDate(getJewishYear(), TISHREI, 22)
            val endDate = JewishDate(getJewishYear(), NISSAN, 15)
     ...
     */  // (forRemoval=true) // add back once Java 9 is the minimum supported version
    @get:Deprecated(
        """Use {@link TefilaRules#isMoridHatalRecited(JewishCalendar)} instead. This method will be
	          removed in the v3.0 release.
